...
# Helper: Save the model weight.
checkpointer = ModelCheckpoint(
    filepath=os.path.join('data', 'checkpoints', 'resnetUCFWeight.{epoch:03d}-{val_loss:.2f}.hdf5'),
    verbose=1,
    save_best_only=True)
...
def main(weights_file):
    ...
    if weights_file is None:
        print("Loading first network from imagenet weights.")
        # Get and train the top layers.
        model = freeze_all_but_top(model)
        model = train_model(model, 10, generators)
    else:
        print("Loading saved model weights: %s." % weights_file)
